...
graph TD
    subgraph Inputs
        A["Input Private Key Hex"];
        B["Input Message Hash H256"];
    end

    subgraph EthersRsProcess [ethers-rs Library Operations]
        direction LR
        C["1 Create LocalWallet"];
        D{"Derived Signer Addr"};
        E["2 Sign Hash"];
        F["Signature Object (r,s,v)"];
        G["3 Extract Components"];
        H["4 Verify Signature (Opt)"];
    end

    subgraph Outputs
        I["Output r U256"];
        J["Output s U256"];
        K["Output v_parity (0/1)"];
